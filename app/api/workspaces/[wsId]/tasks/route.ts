...
export async function POST(
  ...
  // Use provided title/description directly if no AI prompt
  let title = body.title || '新任务';
  let description = body.description || '';
  let agentConfig = body.agentConfig || {
  ...
  let generationStatus = 'direct';

  // AI-assisted task creation if prompt provided
  if (body.prompt) {
    try {
...
Return only valid JSON, no markdown formatting.`,
      ...
      log.warn('AI task generation failed, using user input as title:', err);
